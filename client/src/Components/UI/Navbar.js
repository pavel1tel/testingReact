...
export const Navbar =  (props) => {
    ...
    useEffect(() => {
        ...
        console.dir({authToken, username, isLogout})
        setKey(true)
        try {
    ...
    const handleLogout = () => {
        ...
        }).then(() => setLogout(true)).then();
    }

    if (isLogout && false) {
        new Promise(resolve => {
            resolve(setLogout(false))
        }).then(ret =  (<Redirect to='/accounts/login'/>));
        return ret;
    } else {
        ret = (
                    ...
                    <div className="form-inline my-2 my-lg-0">
                        {authorized ? (
                            <div>
                                ...
                                <span>&nbsp;|&nbsp;</span>
                                <span className="nav-text"><Link to="/accounts/login" onClick={handleLogout}>Log Out</Link></span>
                            </div>
                        ) : (
                            <div>
